fixedshareCalib <- function(y, experts, grid.eta = 1, grid.alpha = 10^(-4:-1), awake = NULL, loss.type = "square", 
  ...
  for (t in 1:T) {
    ...
    # Weights, prediction forme by FixedShare(eta[t],alpha[t]) where par[t,] = c(eta[t],alpha[t]) are the
    # parameters calibrated online
    weights[t, ] <- wpar[, bestpar[1], bestpar[2]] * awake[t, ]/sum(wpar[, bestpar[1], bestpar[2]] * 
      awake[t, ])
