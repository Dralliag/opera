# Instantaneous loss suffered by a prediction The function \code{lossPred} computes the loss of a
# prediction \code{x} of an obstervation \code{y} knowing that we want to correct the error
# commited by the prediction \code{pred}.  It is used in the mixing aggregation rule (see
# \code{\link{mixture}}) to compute the loss of the experts at each instance.  We can choose
# \code{pred} to be the prediction of \code{y} outputed by the aggregation rule.  @param x A vector
...
lossPred <- function(x, y, pred = NULL, loss.type = "square", loss.gradient = FALSE) {
  ...
  if (npred > 1 && nx > 1) {
    if (!loss.gradient) {
      ...
      if (loss.type$name == "square") 
        l <- 2 * t(matrix(rep(pred - y, nx), ncol = nx)) * matrix(rep(x, npred), ncol = npred) else if (loss.type$name == "absolute") 
        l <- t(matrix(rep(sign(pred - y), nx), ncol = nx)) * matrix(rep(x, npred), ncol = npred) else if (loss.type$name == "percentage") 
        l <- matrix(rep(x, npred), ncol = npred)/y * t(matrix(rep(sign(pred - y), nx), ncol = nx)) else if (loss.type$name == "pinball") 
        l <- t(matrix(rep((y < pred) - loss.type$tau, nx), ncol = nx)) * matrix(rep(x, npred), 
          ncol = npred)
    }
  } else {
    if (!loss.gradient) {
