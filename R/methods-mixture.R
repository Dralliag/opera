...
plot.mixture <- function(x, ...) {
  ...
  if (x$model == "Ridge") {
    ...
    matplot(x$weights, type = "l", xlab = "Time steps", ylab = "Weights", lty = 1:5, 
      col = 1:8, main = "Evolution of the weights associated with the experts", 
      ...)
    ...
    plot(c(1), type = "l", col = 1:8, lwd = 2, axes = F, xlim = c(1, T), ylim = c(0, 
      1), ylab = "", xlab = "", main = "Evolution of the convex weights associated with the experts")
  ...
  boxplot(x$weights, main = "Box plot of the weights associated with the experts")
  mtext(side = 2, text = "Weights", line = 1.8, cex = 1)
