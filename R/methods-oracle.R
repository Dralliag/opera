...
print.oracle <- function(x, ...) {
  ...
  cat("\n")
  print(signif(summary(x)$losses,digits=2))
}
...
summary.oracle <- function(object, ...) {
  ...
  if (object$model == "linear" || object$model == "convex") {
    ...
    x <- summary(oracle(object$Y,object$experts, model ="expert", 
      loss.type = object$loss.type), awake = object$awake)
...
plot.oracle <- function(x, ...) {
  ...
  if (x$model == "expert") {
    ...
    plot(c(x$loss.experts,err.unif)[idx.sorted], xlab = "Experts", ylab = paste(x$loss.type, "loss"), 
      main = "Average loss suffered by the experts", axes = F, pch = 3, col = my.col, lwd = 2)
    axis(1, at = 1:(K+1), labels = FALSE)
    mtext(at = 1:(K+1), text = c(names(x$experts), "Uniform")[idx.sorted], 
          side = 1, las = 2, col = my.col, line = .8)
    axis(2)
    box()  
  }
  ...
  if (x$model == "convex" || x$model == "linear") {
    ...
    par(mar = c(4,4,2,2), mgp = c(3,1,0))
    plot(c(x$loss.experts,err.unif,x$loss)[idx.sorted], xlab = "Experts", ylab = paste(x$loss.type, "loss"), 
         main = "Average loss suffered by the experts", axes = F, pch = 3, col = my.col, lwd = 2)
