...
test_that("EWA is ok", {
  ...
  #expect_null(predict(m))
  #e <- c(0.3,0.5)
  #expect_equal(c(predict(m,e)), sum(c(e)*c(m$coefficients)))
  ...
  eta = 0.5
  m.fixed = mixture(Y = Y, experts = X, model = "EWA", 
  ...
  m1 = mixture(Y = Y[1:10], experts = X[1:10,], model = "EWA", parameters = list(grid.eta = grid.eta), awake = awake[1:10,])
...
test_that("Fixed-share is ok", {
  ...
  eta = 2
  alpha = 0.01
  m.fixed = mixture(Y = Y, experts = X, model = "FS", 
                    parameters = list(eta = eta, alpha = alpha), 
                    loss.type = possible_loss_type[i.loss], 
                    coefficients = w0)
  idx.eta  = which(m$parameters$grid.eta == eta)
  ...
  grid.eta = runif(5)
  grid.alpha = runif(3)
  m = mixture(Y = Y, experts = X, model = "FS", parameters = list(grid.eta = grid.eta, grid.alpha = grid.alpha), awake = awake)
  expect_equal(sum(!(grid.eta %in% m$parameters$grid.eta)),0)
...
test_that("Ridge is ok", {
  ...
  grid.lambda = runif(3)
  m = mixture(Y = Y, experts = X, model = "Ridge", parameters = list(grid.lambda = grid.lambda, gamma = 100), awake = awake)
...
test_that("MLpol, MLprod, MLewa, and BOA are ok", {
  ...
  w0 = c(0.3, 0.7)
  m = mixture(Y = Y, experts = X, model = "MLprod", 
              coefficients = w0,
              loss.type = possible_loss_type[i.loss])
  expect_true(abs(m$coefficients[1]-0.6)<2e-1)
  expect_equal(m$loss, mean(loss(m$prediction,Y,loss.type = possible_loss_type[i.loss])))
  ...
  m = mixture(Y = Y, experts = X, model = "MLewa", 
              coefficients = w0,
              loss.type = possible_loss_type[i.loss])
  expect_true(abs(m$coefficients[1]-0.6)<2e-1)
  expect_equal(m$loss, mean(loss(m$prediction,Y,loss.type = possible_loss_type[i.loss])))
  ...
  w0 = c(0.3, 0.7)
  m = mixture(Y = Y, experts = X, model = "BOA", 
              coefficients = w0,
              loss.type = possible_loss_type[i.loss])
  expect_true(abs(m$coefficients[1]-0.6)<2e-1)
  expect_equal(m$loss, mean(loss(m$prediction,Y,loss.type = possible_loss_type[i.loss])))
...
test_that("Quantile mixture are ok", {
  ...
  m = mixture(Y = Y, experts = X, 
              model = "EWA", loss.type = l, loss.gradient = FALSE, parameters = list(eta = 10e-1, gamma = 100))
  ...
  m = mixture(Y = Y, experts = X[,c(1,K)], 
              model = "EWA", loss.type = l, parameters = list(gamma = 100))
  expect_equal(m$loss,mean(loss(m$prediction,Y,loss.type = l)))
  expect_less_than(abs(sum(X[1,c(1,K)] * m$coefficients) - X[1,i]), 0.4)
  
  # Fixed share
  m = mixture(Y = Y, experts = X, 
              model = "FS", loss.type = l, loss.gradient = FALSE, 
              parameters = list(eta = 10e-1, alpha = 0.01))
  expect_equal(m$loss,mean(loss(m$prediction,Y,loss.type = l)))
  expect_less_than(abs(sum(X[1,] * m$coefficients) - X[1,i]), 0.4)
  
  m = mixture(Y = Y, experts = X[,c(1,K)], 
              model = "FS", loss.type = l, parameters = list(gamma = 10))
  expect_equal(m$loss,mean(loss(m$prediction,Y,loss.type = l)))
  expect_less_than(abs(sum(X[1,c(1,K)] * m$coefficients) - X[1,i]), 0.4)
  ...
  m = mixture(Y = Y, experts = X, 
              model = "MLprod", loss.type = l, loss.gradient = FALSE)
  expect_equal(m$loss,mean(loss(m$prediction,Y,loss.type = l)))
  expect_less_than(abs(sum(X[1,] * m$coefficients) - X[1,i]), 0.4)
  ...
  m = mixture(Y = Y, experts = X, 
              model = "MLewa", loss.type = l, loss.gradient = FALSE)
  expect_equal(m$loss,mean(loss(m$prediction,Y,loss.type = l)))
  expect_less_than(abs(sum(X[1,] * m$coefficients) - X[1,i]), 0.4)
  ...
  m = mixture(Y = Y, experts = X, 
              model = "BOA", loss.type = l, loss.gradient = FALSE)
  expect_equal(m$loss,mean(loss(m$prediction,Y,loss.type = l)))
  expect_less_than(abs(sum(X[1,] * m$coefficients) - X[1,i]), 0.4)
